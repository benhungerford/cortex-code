...
`build` owns the criteria completely. It loops — implement, then spawn a fresh checker agent that verifies the work in a real browser against the criteria and returns a pass/fail/blocked list, fix everything it returned failing, go again — until a checker round comes back with every criterion and every unresolved QA item passing or blocked. There is no round cap; the loop runs as long as the work is moving. It stops short only when it stalls (the same item fails twice with the checker's observation unchanged), when bot protection trips, or when the fix would require changing something above the ticket's divider — and any of those holds the task at `in-progress` rather than handing off to `qa`, because that state is not ready for a human to sign off and `review` would misrepresent it.

The checker is a separate agent on purpose, and it cannot write files. An agent that verifies its own build is grading a page it already believes is correct; one that starts with no memory of the reasoning, and is given the criteria verbatim rather than the conclusion, is the only thing in the loop that can actually say no. `qa` walks none of the criteria; its question is *was the ticket right?*, not *did the plan get followed?*
