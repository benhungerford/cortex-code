...
When the map has no open questions **and `Not yet specified` is empty**, print:
...
Fog left in `Not yet specified` is an in-scope decision still coming, and a task set drawn over it rests on a guess.
